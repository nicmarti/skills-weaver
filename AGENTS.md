...
- **Model provider boundary (migration complete):** only `internal/llm` may import the OpenRouter SDK (`github.com/OpenRouterTeam/go-sdk`); do not add new provider SDK imports elsewhere. **Phases 1–12 done:** the main agent loop, the three nested agents (rules-keeper, character-creator, world-keeper), every direct utility call (journal/map enrichment, ambient Lyria parameters, biographies, campaign plans, adventure titles) and the sw-dm CLI flags (`--model`, `--nested-model`, `--agent-model`) run on the provider-neutral `llm.Client` (OpenRouter Chat, Sonnet 5 default) over one shared client per process; `cmd/dm`/`cmd/web` require `OPENROUTER_API_KEY` via `llm.LoadConfig`; the web model selector renders from the neutral catalog (`llm.SelectableModels`). **The Anthropic SDK (`github.com/anthropics/anthropic-sdk-go`) is fully removed from the module** (Phase 12) — no imports, no `ANTHROPIC_API_KEY` reads anywhere. **Defect neutralized (Phase 10):** the generated SDK's EventStream swallows terminal body read errors — a stream dying mid-flight was reported as a clean success with a truncated message; `llm.Stream` now surfaces such failures as `mid_stream`/`timeout`/`canceled` via a transport-level read-error holder (a clean end of stream never hides a dead connection). The Advisor feature (Anthropic beta + planned native `openrouter:advisor`) was **removed entirely** — see the Advisor section below. OpenRouter model IDs and key loading are centralized in `internal/llm/models.go` and `internal/llm/config.go`. Evidence: `internal/agent/agent.go`, `internal/agent/agent_manager.go`, `cmd/dm/main.go`; see `docs/openrouter-migration-plan.md`.
...
- Provider keys live in the shell environment (`.envrc` via direnv, not committed): `OPENROUTER_API_KEY` (required for `sw-dm`/`sw-web` gameplay and every LLM utility — nested agents, enrichment, campaign plans, titles, biographies; optional `OPENROUTER_MODEL_DM`, `OPENROUTER_MODEL_NESTED`, `OPENROUTER_MODEL_RULES_KEEPER`, `OPENROUTER_MODEL_CHARACTER_CREATOR`, `OPENROUTER_MODEL_WORLD_KEEPER`, `OPENROUTER_MODEL_FAST`, `OPENROUTER_MODEL_CAMPAIGN`, `OPENROUTER_HTTP_REFERER`, `OPENROUTER_APP_NAME`; see `internal/llm/config.go`), `GEMINI_API_KEY` (Lyria ambient music), `FAL_KEY` (fal.ai images). A human must configure them; agents must never print, log, or commit key values. `ANTHROPIC_API_KEY` n'est plus lue par aucun code depuis la Phase 12 (le SDK Anthropic a été retiré du module).
...
⚠️ **Note de migration (Anthropic → OpenRouter)** : le DM principal, les agents imbriqués (rules-keeper, character-creator, world-keeper) et tous les appels utilitaires (enrichissement, plans de campagne, titres, paramètres ambient, biographies) tournent désormais sur OpenRouter (`llm.Client`, Sonnet 5 par défaut, `OPENROUTER_API_KEY` — Phases 4–5, 7 et 8–12). La fonction Advisor a été **entièrement supprimée** (voir la section « Outil Advisor » ci-dessous). `ANTHROPIC_API_KEY` n'est plus lue par aucun code, et le SDK Anthropic a été retiré du module (Phase 12). Plan, décisions et journal de progression : `docs/openrouter-migration-plan.md`.
