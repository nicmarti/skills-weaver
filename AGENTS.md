...
- **Model provider boundary (migration in progress):** only `internal/llm` may import the OpenRouter SDK (`github.com/OpenRouterTeam/go-sdk`); do not add new provider SDK imports elsewhere. **Phases 4–5 + 7 done:** the main agent loop, the three nested agents (rules-keeper, character-creator, world-keeper) and every direct utility call (journal/map enrichment, ambient Lyria parameters, biographies, campaign plans, adventure titles) run on the provider-neutral `llm.Client` (OpenRouter Chat, Sonnet 5 default) over one shared client per process; `cmd/dm`/`cmd/web` require `OPENROUTER_API_KEY` via `llm.LoadConfig`. The Advisor feature (Anthropic beta + planned native `openrouter:advisor`) was **removed entirely** — see the Advisor section below. **No production code reads `ANTHROPIC_API_KEY` anymore.** The only remaining Anthropic SDK imports are Phase-12 legacy compat with zero runtime callers (`internal/agent`: `model_mapping.go`, `streaming.go`, `GetMessages()` conversion; `internal/llm/anthropic_compat.go`). Remaining before cutover: Phase 8 CLI flags, Phase 9 web model catalog, Phases 11–12. OpenRouter model IDs and key loading are centralized in `internal/llm/models.go` and `internal/llm/config.go`. Evidence: `internal/agent/agent.go`, `internal/agent/agent_manager.go`, `cmd/dm/main.go`; see `docs/openrouter-migration-plan.md`.
