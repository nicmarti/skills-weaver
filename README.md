...
For consistent character appearance across journal images, use character appearance descriptions and reference images with the FLUX PuLID model:
...
./sw-character appearance "Aldric" \
  ...
  --height=tall \
  --hair-color=black \
  --facial-feature=bearded \
  --distinctive="scar across left eye" \
  --armor="plate armor" \
  --weapon=longsword \
  --accessories=shield

# Set reference image (for FLUX PuLID)
./sw-character set-reference "Aldric" path/to/portrait.png

# Generate journal images with character consistency
./sw-image journal "my-adventure" --model=pulid --consistency
```

### How it works

1. **Character appearances** are stored in character JSON files
2. **Reference images** enable FLUX PuLID model for face consistency
3. **Journal image generation** automatically loads party members
4. **Main character** is identified per entry (by name mention or default to party leader)
5. **Short character descriptions** replace long repeated text in prompts

### Models Comparison

| Model | Speed | Cost/image | Character Consistency |
|-------|-------|------------|----------------------|
| `schnell` | ~3s | ~$0.003 | No |
| `banana` | ~5s | ~$0.039 | No |
| `pulid` | ~4s | ~$0.003-0.04 | Yes (requires reference images) |
...
The reference image ensures Aldric's face matches across all images, eliminating the need for detailed descriptions.
