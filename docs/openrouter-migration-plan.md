...
Status (updated 2026-09-24): Phases 1–5, 7–12 implemented (Phase 6 dropped: the Advisor feature was removed entirely). The Anthropic SDK is fully removed from the module — no imports and no `ANTHROPIC_API_KEY` reads anywhere. The main DM loop, the three nested agents and every direct utility call (journal/map enrichment, ambient Lyria parameters, biographies, campaign plans, adventure titles) run on `llm.Client` (OpenRouter Chat, Sonnet 5 default). Phase 0 live evidence: a deterministic in-repo fixture proves the genuine mid-stream-incident behavior (simulated TCP reset surfaced as mid_stream, no silent truncation); a genuine provider mid-stream error has still never been observed live — desirable, but no longer blocking for that scenario. Remaining before cutover: run one same-version reference adventure through the migrated engine (Phase 12 acceptance item). See "Decision gates before cutover" below.
