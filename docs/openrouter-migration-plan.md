...
Status (updated 2026-09-23): Phases 1–5 implemented; the main DM loop and the three nested agents run on `llm.Client` (OpenRouter Chat, Sonnet 5 default) behind fake-client tests, and `cmd/dm`/`cmd/web` require `OPENROUTER_API_KEY`. The Anthropic beta Advisor execution path was removed with Phase 5 (native `openrouter:advisor` lands in Phase 6 behind `SW_ADVISOR_ENABLED`). Remaining Anthropic callers: web campaign/title utilities, `internal/ai`, `internal/ambient`, `internal/charactersheet` (Phase 7) — the legacy key is only read for those unmigrated paths. Phase 0 live evidence remains incomplete (Advisor transcript and genuine provider mid-stream error). See "Decision gates before cutover" below.
...
Exit criterion: **met** — Rules Keeper, Character Creator and World Keeper run through a fake shared `llm.Client` with read-only tools, policy-bounded iterations, silent briefing without tools, provider-error handling without partial-history commit, and persisted legacy/new conversations including the new OpenRouter metrics. `SW_ADVISOR_ENABLED` currently gates nothing on the nested path (the Anthropic beta advisor was removed; the native advisor is Phase 6). No `scenario-critic` test or Anthropic client factory remains in the active nested runtime (`internal/agent/advisor.go` and `mock_anthropic.go` deleted; `model_mapping.go`/`streaming.go`/`GetMessages()` remain only as Phase-12-removable legacy compat).
