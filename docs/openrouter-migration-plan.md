...
Status (updated 2026-09-23): Phases 1–5 implemented; the main DM loop and the three nested agents run on `llm.Client` (OpenRouter Chat, Sonnet 5 default) behind fake-client tests, and `cmd/dm`/`cmd/web` require `OPENROUTER_API_KEY`. The Advisor feature (Anthropic beta path **and** the planned native `openrouter:advisor`) was removed entirely — see the dropped Phase 6. Remaining Anthropic callers: web campaign/title utilities, `internal/ai`, `internal/ambient`, `internal/charactersheet` (Phase 7) — the legacy key is only read for those unmigrated paths. Phase 0 live evidence remains incomplete (genuine provider mid-stream error; the Advisor transcript item is moot since the feature was removed). See "Decision gates before cutover" below.
