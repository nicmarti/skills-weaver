...
func TestGetModelDisplayName(t *testing.T) {
	tests := []struct {
		...
		{"sonnet 4.6", anthropic.ModelClaudeSonnet4_6, "claude-sonnet-4-6"},
...
func TestDefaultNestedAgentModel(t *testing.T) {
	// Verify default is Sonnet 4.6
	if DefaultNestedAgentModel != anthropic.ModelClaudeSonnet4_6 {
		t.Errorf("DefaultNestedAgentModel = %v, want %v",
			DefaultNestedAgentModel, anthropic.ModelClaudeSonnet4_6)
	}
}
